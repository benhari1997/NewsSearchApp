...
class NewsApp(QDialog):
    ...
    def createConnectionBox(self):
        """This function creates the connection box buttons, triggers and layouts"""
    ...
    def createSearchBox(self):
        """This function creates the search box buttons, triggers and layouts"""
    ...
    #triggered functions
    def connect(self) :
        """if the id and the password is correct it connects to the database 
        and allows the user to use the other features, if its not it displays a message error"""

        try :
            #database connection
            self.news = database_conn(self.lineEdit_user.text() ,self.lineEdit_pw.text())
    ...
    def search(self):
        ...
        search_results = list(self.news.find( { 'Article Title' : { "$regex": '.*'+
        ...
        self.SearchTableWidget.setRowCount(row_count)
            #headers
        self.SearchTableWidget.setHorizontalHeaderLabels((list(search_results[0].keys())[1:]))
            #filling the cells
    ...
    def updateNews(self):
        """This function extracts the news from the main Daily mail page 
        and stores and display them on the app"""
        ...
        self.NewsTableWidget.setRowCount(row_count)
            #headers
        self.NewsTableWidget.setHorizontalHeaderLabels((list(today_news[0].keys())))
            #filling the cells
        for row in range(row_count):  
            for column in range(column_count-1):
                item = (list(today_news[row].values())[column])
                self.NewsTableWidget.setItem(row, column, QTableWidgetItem(str(item)))
